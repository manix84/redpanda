...
class Ctcp
      ...
      ctcp :clientinfo
      def ctcp_version(m)
        m.ctcp_reply "redpanda (Cinch v#{Cinch::VERSION}) - jak spalding" if reply_to_ctcp?(:version)
      end
      ...
      def ctcp_source(m)
        m.ctcp_reply "http://github.com/jakspalding/redpanda" if reply_to_ctcp?(:source)
      end
